...
$URL = $session->get('absoluteURL')."/index.php?q=/modules/Mastery Transcript/credits_manage_edit.php&masteryTranscriptCreditID=$masteryTranscriptCreditID&masteryTranscriptDomainID=$masteryTranscriptDomainID&search=$search";
...
if (isActionAccessible($guid, $connection2, '/modules/Mastery Transcript/credits_manage_edit.php') == false) {
    ...
    if (empty($data['name']) || empty($data['level']) || empty($data['active'])) {
        $URL .= '&return=error1';
        header("Location: {$URL}");
        exit;
    }
